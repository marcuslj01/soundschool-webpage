...
import { FLPInput } from "@/lib/types/FLP";
import ReactMarkdown from "react-markdown";
import remarkGfm from "remark-gfm";
import Button from "./Button";
import { useAuth } from "@/contexts/AuthContext";

interface FLPUploadFormProps {
  onClose: () => void;
  onBack: () => void;
}

function FLPUploadForm({ onClose, onBack }: FLPUploadFormProps) {
  const { user: currentUser } = useAuth();
  const [form, setForm] = useState<FLPInput>({
    name: "",
    description: "",
    price: 0,
    root: "C",
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.ok) {
        alert("FLP uploaded successfully");
        onClose();
      } else {
    ...
    <div className="bg-white rounded-lg shadow-lg w-full max-w-lg p-8 relative">
      <div className="flex justify-between items-center">
        <div className="flex flex-row gap-2">
          ...
          <h1 className="text-2xl font-bold">Upload FLP</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="flex flex-col gap-2">
          ...
          <input
            type="url"
            name="video_url"
            placeholder="YouTube Video URL (thumbnail will be auto-generated)"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
            <input
              type="text"
              name="name"
              placeholder="Name (empty = YT-title)"
              onChange={handleChange}
              className="w-full px-3 py-2 bg-gray-50 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
            <input
